...
def load_questions(n=100, seed=42):
    pool = _load_pool()
    rng = random.Random(seed)
    return rng.sample(pool, min(n, len(pool)))


def load_age_stratified(n_per_group=100, seed=42):
    pool = _load_pool()
...
def load_prefiltered_questions(n=100, seed=42):
...
def load_id_ood_questions(n_id=100, n_ood=50, seed=42
                          ):
    pool = _load_pool()
